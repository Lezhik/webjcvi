...
public class WebJcviMcpTools {
    ...
    public WebJcviMcpTools(FileStorageService storage, DnaReportService reports) {
        this(storage, reports, new ScratchTape(), new BannerSplitter(), new PairDrift(),
                ...
                new CloneScan(), new PrefixGroup(), new KeyWidth(), new ForkScan(), new AffixScan(), new LaneScan(), new RowScan(), new CliffScan(), new RunwayScan(), new RiseScan(), new MajorityScan(), new NearScan(), new ResidueScan(), new DupScan(), new LingerScan(), new LogAnalysisService());
    }
    ...
    @Tool(name = "profile_cliffs", description = "Find where newline-delimited colliding lines first fork, and the smallest prefix k whose unique share is at least 0.99. Newlines are kept. Not the DNA file and not a project path.")
    public String profileCliffs(
            @ToolParam(description = "Arbitrary text to scan") String text) {
        return run(() -> cliffs.profile(text).summary());
    }

    @Tool(name = "profile_runways", description = "Measure the layout runway of newline-delimited lines: cliffAt minus forkAt. If stretched (runway greater than 20), start an identifier at cliffAt, not at forkAt. Newlines are kept. Not the DNA file and not a project path.")
    public String profileRunways(
            @ToolParam(description = "Arbitrary text to scan") String text) {
    ...
    @Tool(name = "profile_majorities", description = "Find the smallest prefix length whose unique share is at least 0.5 on newline-delimited lines. If pastRise (majorityAt greater than riseAt), start an identifier there rather than at the steepest jump. Not the DNA file and not a project path.")
    public String profileMajorities(
            @ToolParam(description = "Arbitrary text to scan") String text) {
    ...
    @Tool(name = "profile_dups", description = "Split leftover colliding lines after the 0.90 near-unique width into exact copies versus twins that still fork. If mostlyCopies (copyShare greater than 0.5), dedupe rather than walking splitLag. Not the DNA file and not a project path.")
    public String profileDups(
            @ToolParam(description = "Arbitrary text to scan") String text) {
        return run(() -> dups.profile(text).summary());
    }

    @Tool(name = "profile_lingers", description = "Bin leftover colliding lines after the 0.90 near-unique width by splitLag. If longTail (lagLong greater than 0), leftover persists past DNA's two-base climb even when mostlyCopies. Not the DNA file and not a project path.")
    public String profileLingers(
            @ToolParam(description = "Arbitrary text to scan") String text) {
        return run(() -> lingers.profile(text).summary());
    }
    ...
    private static String run(ToolAction action) {
        try {
            return action.execute();
                 ...
                 | FuzzyException | ContrastException | MirrorException | SeamException | PhaseException | FrameException | CloneException | PrefixException | KeyException | ForkException | AffixException | LaneException | RowException | CliffException | RunwayException | RiseException | MajorityException | NearException | ResidueException | DupException | LingerException e) {
